...
def monty_hall_sim(numdoors, numgames):
    ...
    plt.show()
    print("Results:")
    print("Number of wins from staying " + game_history_stay[-1])
    print("Number of wins from staying " + game_history_change[-1])
    
def monty_hall_sim_interactive(guess, change):
    doorStrings = ["Door 1","Door 2","Door 3"]
    number_of_doors = 3
    guessNum = doorStrings.index(guess)
    door_with_car = rd.randint(0,2)
    door_to_keep_shut = door_with_car
    if(guessNum == door_with_car):
        while(guessNum == door_to_keep_shut):
            door_to_keep_shut = rd.randint(0,2)
    door_to_reveal = number_of_doors - door_to_keep_shut - guessNum
    # monty reveals shut door
    time.sleep(1.5)
    print("You tell Monty Hall that you choose door " + str(guessNum + 1) + "\n")
    time.sleep(1.5)
    print("Monty hall reveals that door " + str(door_to_reveal + 1) + " has a goat behind it!\n")
    guess_statement = "keep"
    if change == "Yes":
        guess_statement = "change"
        guessNum = door_to_keep_shut
    time.sleep(1.5)
    print("Since you chose to " + guess_statement + " your door, you choose door " + str(guessNum + 1) +"\n")
    time.sleep(1.5)
    if guessNum == door_with_car:
...
def radar_dectector(detections):
    ...
    events = {"Miss": 0, "Detected": 0, "Nothing detected": 0, "False alarm": 0}
    ...
    labels = [events.get("Miss"), events.get("Detected"), events.get("Nothing detected"),events.get("False alarm")]
    plt.text(x = -0.1, y = labels[0] + 0.4, s = labels[0], size = 20)
    plt.text(x = 0.90, y = labels[1] + 0.4, s = labels[1], size = 20)
    plt.text(x = 1.90, y = labels[2] + 0.4, s = labels[2], size = 20)
    plt.text(x = 2.90, y = labels[3] + 0.4, s = labels[3], size = 20)
